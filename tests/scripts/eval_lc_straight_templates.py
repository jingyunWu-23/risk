...
from tests.envs.action_adapter import ActionAdapter, DiscreteDrivingAction
...
def resolve_ego_checkpoints(value: str) -> List[Tuple[str, Optional[Path]]]:
...
def load_ego_policy(path: Optional[Path], no_cuda: bool, seed: int):
    ...
    raise RuntimeError(
        "Ego checkpoint loading is not available in this project yet. "
        "Run LC template evaluation with --ego-checkpoints omitted to use fixed ego actions."
    )
...
def main():
    ...
    ego_items = resolve_ego_checkpoints(args.ego_checkpoints)
