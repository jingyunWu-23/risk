"""25-D low-dimensional observation builder for straight-road template tests."""

from __future__ import annotations

import numpy as np

from tests.envs.vehicle_state import longitudinal_gap
...
class LowDimObservationBuilder:
    ...
    def build_vehicle_observation(self, scenario, vehicle) -> np.ndarray:
        if not getattr(vehicle, "active", True) or not getattr(vehicle, "physical_active", True):
            return np.zeros(self.state_dim, dtype=np.float32)
